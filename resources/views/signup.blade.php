...
<body class="main-col">
        ...
        <div class="row d-flex justify-content-center mt-1">
            <div class="col-8">
                <div class="login-form main-col text-center p-5 ">
                    <h1 class="txt-col pb-4 ">{{ trans('signup.Registration_webpage') }}</h1>
                    <form method="POST"  onsubmit="return validateForm()" action="{{route('signup')}}" enctype="multipart/form-data">
                        @csrf
                        <input type=" text" name="full_name" id="full_name" placeholder='{{ trans('signup.full_name') }}'
                            class="w-100 bg-transparent form-control text-white mb-3">
                        <input type="text" name="user_name" id="user_name" placeholder='{{ trans('login.user_name') }}'
                            class="w-100 bg-transparent form-control text-white mb-3">
                        <div class="d-flex">
                            ...
                            <button type="button" class="w-25 rounded btn btn-form h-100 ms-2"
                                onclick="getActor(birthdate.value)">{{trans('signup.actors')}}
                            </button>
                        ...
                        <input type=" tel" id="phone" name="phone"
                            class="w-100 bg-transparent form-control text-white mb-3" placeholder='{{ trans('signup.phone') }}'>

                        <input type="text" id="address" name="address"
                            class="w-100 bg-transparent form-control text-white mb-3" placeholder='{{ trans('signup.address') }}'>

                        <input type="password" name="password" id="password" placeholder='{{ trans('signup.password') }}'
                            class="w-100 bg-transparent form-control text-white mb-3">

                        <input type="password" id="confirm_password" name="confirm_password"
                        placeholder='{{ trans('signup.confirmPassword') }}' class="w-100 bg-transparent form-control text-white mb-3">
                        <input type="file" id="user_image" name="user_image"
                            class="w-100 bg-transparent form-control text-white mb-3">

                        <input type="email" name="email" id="email" placeholder='{{ trans('signup.email') }}'
                        ...
                        <button class="w-100  rounded btn btn-form" type="submit" name="submitptn" id="signupbtn">{{trans('signup.signUp')}}</button>
                        <p class="text-white pt-4">{{trans('signup.haveAccount')}}<a href="{{ url('login') }}" class="text-white"
                                id="signin"> {{trans('signup.signIn')}}</a></p>
                    </form>
    ...
    <script src="{{ asset('js/API-Ops.js') }}"></script>
</body>
